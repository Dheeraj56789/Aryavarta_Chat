// High-Performance Zero-Dependency Fuzzy String & Contact Matcher
...
// 2. Normalized String Similarity (0.0 to 1.0)
...
// 3. Match Spoken Contact Name Against Contact List
export function matchContact(spokenName, contacts = []) {
    ...
    const scored = contacts.map((contact) => {
        ...
        // Spoken name exactly equals first name or last name
        else if (nameTokens.includes(cleanSpoken)) {
            maxScore = 0.95;
        }
        // Fullname contains spoken name as exact substring
        else if (fullname.includes(cleanSpoken) || username.includes(cleanSpoken)) {
            maxScore = 0.90;
        } else {
            // Check direct full string similarity
            ...
            // Check token-level similarities (e.g. "Rahuul" vs "Rahul")
    ...
    // Filter and sort candidates with score >= 0.65
    ...
    // Check if there are multiple close matches requiring clarification
    // e.g., Spoken "Rahul", Contacts: "Rahul Sharma" (0.95) & "Rahul Verma" (0.95)
