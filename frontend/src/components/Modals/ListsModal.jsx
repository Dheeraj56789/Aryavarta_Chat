...
const INITIAL_LISTS = [
  ...
  { id: "work", name: "Work", members: [] }
];

const ListsModal = ({ onClose }) => {
  const { allUsers } = useChatContext();
  const [lists, setLists] = useState(() => {
    try {
      const saved = localStorage.getItem("aryavarta_custom_user_lists");
  ...
  const saveLists = (updated) => {
    setLists(updated);
    localStorage.setItem("aryavarta_custom_user_lists", JSON.stringify(updated));
  };
  ...
  const realContacts = (Array.isArray(allUsers) ? allUsers : []).filter((u) => {
      ...
      <div className="bg-[#111b21] border border-slate-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col text-slate-100 max-h-[85vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/80 bg-[#16202a]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Lists</h3>
              <p className="text-[11px] text-slate-400">Manage people and groups</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-3 border-b border-slate-800/60 bg-slate-900/50 flex items-center gap-2 overflow-x-auto scrollbar-none">
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-emerald-400 transition-colors flex-shrink-0 cursor-pointer"
        ...
        {showAddList && (
          <form onSubmit={handleCreateList} className="p-3 bg-slate-800/40 border-b border-slate-800 flex gap-2 animate-fadeIn">
            <input
              type="text"
              placeholder="New list name (e.g. Work, College)..."
              value={newListName}
            ...
            <button
              type="submit"
              className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 rounded-xl text-xs font-bold transition-colors cursor-pointer"
            >
              Create
            </button>
          </form>
